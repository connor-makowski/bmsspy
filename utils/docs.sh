...
cp README.md bmsspy/__init__.py
sed -i '1s/^/\"\"\"\n/' bmsspy/__init__.py
...
function generate_docs() {
    ...
    if [ $INPUT_VERSION != "./" ]; then
        if [ $INPUT_VERSION != $VERSION ]; then
            pip install "./dist/bmsspy-$INPUT_VERSION.tar.gz"
        fi
    fi
    pdoc -o ./docs/$INPUT_VERSION -t ./doc_template bmsspy
}
