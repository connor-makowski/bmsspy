...
for data in tests:
    ...
    assert sorted([v for k, v in quicksplit_dict_output['lower'].items()]) == sortsplit_output['lower'], f"Mismatch for Quickselect Dict lower: {quicksplit_output['lower']} != {sortsplit_output['lower']}"
    assert sorted([v for k, v in quicksplit_dict_output['higher'].items()]) == sortsplit_output['higher'], f"Mismatch for Quickselect Dict higher: {quicksplit_output['higher']} != {sortsplit_output['higher']}"
    ...
    qsd_time = pamda_timer(quicksplit_dict, iterations = 3).get_time_stats(data=dict_data)
    print(f"n={len(data)}, QS Time: {qs_time['avg']:.4f} ms, SS Time: {ss_time['avg']:.4f} ms, Speedup: {ss_time['avg']/qs_time['avg']:.4f}x, QSD Time: {qsd_time['avg']:.4f} ms")