...
\While{$Heap\neq\emptyset$}{
  ...
  $S'\leftarrow S'\cup\{u\}$\;
  \ForEach{$(v,w)\in G[u]$}{
    {\color{ForestGreen}$d'\leftarrow \textit{cdist}[u] + w + counter\_value + \textit{edge\_adj}[u][v]$\tcp*{Unique distance}\label{alg:bmssp_base_case_modified_distance}}
    \If{$d'\le \textit{dist}[v]$ \textbf{and} $d'<B$}{
      ...
      {\color{ForestGreen}$\textit{cdist}[v]\leftarrow \textit{cdist}[u] + w + counter\_value$\tcp*{Update counter distance}\label{alg:bmssp_base_case_counter_dist_update}}
