...
{\color{Sepia}$\textit{cdist}[0..|G|-1] \leftarrow \infty$ \tcp*{Counter distance matrix}\label{alg:bmssp_solve_cdist}}
{\color{ForestGreen}$\textit{pred}[0..|G|-1] \leftarrow -1$ \tcp*{Track predecessors for path reconstruction}\label{alg:bmssp_solve_pred}}
...
\BlankLine
{\color{blue}$k \leftarrow \lfloor \log_2(|G|)^{1/3} \rfloor$ \tcp*{Use $log_2$}\label{alg:bmssp_solve_k}}

{\color{blue}$t \leftarrow \lfloor \log_2(|G|)^{2/3} \rfloor$ \tcp*{Use $log_2$}\label{alg:bmssp_solve_t}}
{\color{blue}$l \leftarrow \left\lceil \dfrac{\log_2(|G|)}{t} \right\rceil$\  \tcp*{Use $log_2$}\label{alg:bmssp_solve_l}}
...
{\color{Sepia}
  \ForEach{$i \in \textit{dist}$}{
    \If{$\textit{dist}[i] \neq \infty$}{
      $\textit{dist}[i] \leftarrow \text{round}(\textit{dist}[i], p)$\tcp*{Convert back to original precision}\label{alg:bmssp_solve_precision_adjustment}
    }
  }
}
