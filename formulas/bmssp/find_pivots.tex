...
$W\leftarrow S$; {\color{Blue}$W_{\mathrm{prev}}$}$\leftarrow S$\;
\For{$1$ \KwTo $k$ iterations}{
  {\color{Blue}$W_{\mathrm{curr}}$}$\leftarrow\emptyset$\;
  \ForEach{$u\in$ {\color{Blue}$W_{\mathrm{prev}}$}}{
    \ForEach{$(v,w)\in G[u]$}{
      ...
      \If{$d'\le \textit{dist}[v]$}{
          ...
          {\color{Sepia} $\textit{cdist}[v]\leftarrow \textit{cdist}[u] + w + counter$\tcp*{Update counter distance}\label{alg:bmssp_find_pivots_counter_dist_update}}
        \If{$d'<B$}{{\color{Blue}$W_{\mathrm{curr}}$}$\leftarrow$ {\color{Blue}$W_{\mathrm{curr}}$}$\cup\{v\}$}
      }
    }
  }
  $W\leftarrow W\cup$ {\color{Blue}$W_{\mathrm{curr}}$}\;
  \If{$|W|>k|S|$}{\Return{$(S,W)$}}
  {\color{Blue}$W_{\mathrm{prev}}$}$\leftarrow$ {\color{Blue}$W_{\mathrm{curr}}$}\;
}
\BlankLine
{\color{Bittersweet}
...
\ForEach{$u\in W$}{
  \ForEach{$(v,w)\in G[u]$}{
    \If{$v\in W$ \textbf {and} {\color{Blue}$\textit{pred}[v] = u$}\label{alg:bmssp_find_pivots_pred_not_dist}}{
