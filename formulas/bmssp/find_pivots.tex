...
{\color{Bittersweet}
$F \leftarrow \{u \mapsto \emptyset : u \in W\}$\tcp*{EFPC: initialize empty adjacency lists for forest $F$}\label{alg:bmssp_find_pivots_forest_init}
$F^{ID} \leftarrow \{u \mapsto 0 : u \in W\}$\tcp*{EFPC: initialize in-degree counter $F^{ID}$}
\BlankLine
\ForEach{$u \in W$}{
  \ForEach{$(v,w) \in G[u]$}{
    \If(\tcp*[f]{EFPC: add edge $v$ if reached from $u$ \label{alg:bmssp_find_pivots_pred_in_forest}}){$v \in W$ \textbf{and} {\color{Blue}$\textit{pred}[v] = u$}}{
      $F[u] \leftarrow F[u] \cup \{v\}$\tcp*{EFPC: record child $v$ under parent $u$ in $F$}
      $F^{ID}[v] \leftarrow F^{ID}[v] + 1$\tcp*{EFPC: increment in-degree of $v$}
    }
  }
}\label{alg:bmssp_find_pivots_forest_loop_end}
$P \leftarrow \emptyset$\;
\ForEach{$u \in S$}{\label{alg:bmssp_find_pivots_roots_start}
  \If(\tcp*[f]{EFPC: root node in forest $F$}){$F^{ID}[u] = 0$}{
