\begin{algorithm}[!htbp]
\SingleSpacedXI
\caption{Recursive BMSSP (\texttt{BmsspSolver.recursive\_bmssp})}
\label{alg:bmssp_recursive_bmssp}
\SetAlgoLined
\KwIn{depth $l$, bound $B$, frontier $S$}
\KwOut{$(B_{\mathrm{comp}},U)$}

\If{$l=0$}{
  \Return $(B_{\mathrm{comp}}, U) \leftarrow$ \textsc{BaseCase}($B,S$)\;
}

$(P,W) \leftarrow$ \textsc{FindPivots}($B,S$)\;
$D \leftarrow$ new \textsc{BmsspDataStructure}($M,B$) with $M=2^{(l-1)t}$\;
\ForEach{$p \in P$}{$D.\textsc{Insert}(p, \textit{dist}[p])$}

{\color{Fuchsia}$B_{\mathrm{comp}}$} $\leftarrow \min(\{\textit{dist}[p] : p \in P\}\cup\{B\})$; $U \leftarrow \emptyset$\;

\While{$|U| < k \cdot 2^{lt}$ and $\neg D.\textsc{IsEmpty}()$}{
  ({\color{Fuchsia}$B_{\mathrm{curr}}$},{\color{Fuchsia}$S_{\mathrm{curr}}$}) $\leftarrow D.\textsc{Pull}()$\;
  ({\color{Fuchsia}$B_{\mathrm{comp}}$},{\color{Fuchsia}$U_{\mathrm{curr}}$}) $\leftarrow$ \textsc{RecursiveBMSSP}($l-1$,{\color{Fuchsia}$B_{\mathrm{curr}}$},{\color{Fuchsia}$S_{\mathrm{curr}}$})\;
  $U \leftarrow U \cup$ {\color{Fuchsia}$U_{\mathrm{curr}}$}\;
  $K \leftarrow \emptyset$\;

  \ForEach{$u \in$ {\color{Fuchsia}$U_{\mathrm{curr}}$}}{
    \ForEach{$(v,w) \in G[u]$}{
      {\color{ForestGreen}$d' \leftarrow \textit{cdist}[u]+w+counter\_value+\textit{edge\_adj}[u][v]$\tcp*{Unique distance}}
      \If{$d' \leq \textit{dist}[v]$}{
        $\textit{dist}[v]\leftarrow d'$\;
        {\color{ForestGreen}$\textit{pred}[v]\leftarrow u$\tcp*{Update predecessor}\label{alg:bmssp_recursive_bmssp_pred}}
        {\color{ForestGreen}$\textit{cdist}[v]\leftarrow \textit{cdist}[u]+w+counter\_value$\tcp*{Update counter distance}}
        \uIf{{\color{Fuchsia}$B_{\mathrm{curr}}$}$ \leq d' < B$}{ $D.\textsc{Insert}(v,d')$ }
        \ElseIf{{\color{Fuchsia}$B_{\mathrm{comp}}$}$ \leq d' < $ {\color{Fuchsia}$B_{\mathrm{curr}}$}}{ $K \leftarrow K \cup \{(v,d')\}$ }
      }
    }
  }

  $D.\textsc{BatchPrepend}(K \cup \{(x,\textit{dist}[x]) : x \in$ {\color{Fuchsia}$S_{\mathrm{curr}}$}$, ${\color{Fuchsia}$B_{\mathrm{comp}}$}$ \leq \textit{dist}[x] < $ {\color{Fuchsia}$B_{\mathrm{curr}}$}$\})$\;
}
\Return $({\color{Fuchsia}B_{\mathrm{comp}}},U \leftarrow U \cup \{x \in W : \textit{dist}[x] < {\color{Fuchsia}B_{\mathrm{comp}}}\})${\color{Fuchsia}\tcp*{Unnecessary $\min$ omitted}}\label{alg:bmssp_recursive_bmssp_return}

\end{algorithm}