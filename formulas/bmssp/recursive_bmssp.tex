...
$M \leftarrow 2^{(l-1)t}$\;
\textnormal{\textsc{BmsspDataStructure}}($M,B$)
  ...
  \ForEach{$u \in$ {\color{Blue}$U_{\mathrm{curr}}$}}{
    \ForEach{$(v,w) \in G[u]$}{
      ...
      \If{$d' \leq \textit{dist}[v]$}{\label{alg:bmssp_recursive_relax}
        ...
        \ElseIf {{\color{Blue}$B_{\mathrm{comp}}$}$ \leq d' < $ {\color{Blue}$B_{\mathrm{curr}}$}}{ {\color{Blue}$K \leftarrow K \cup \{v\}$\tcp*{FIN: Store K as set of of vertices}} } 
      }\label{alg:bmssp_recursive_collectK}
    }
  }
}
$D.\textsc{BatchPrepend}(\{(i,\textit{dist}[i]) : i \in K \cup \{j : j \in$ {\color{Blue}$S_{\mathrm{curr}}$}$, ${\color{Blue}$B_{\mathrm{comp}}$}$ \leq \textit{dist}[j] < $ {\color{Blue}$B_{\mathrm{curr}}$}$\}\})$\;\label{alg:bmssp_recursive_batchprepend}
