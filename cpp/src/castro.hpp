...
// Code copied with minor adaptations from CASTRO
// See: https://github.com/lcs147/bmssp/commit/51b54e256d0a7166b3d12f6a5d54377d29554f9f
// See: https://github.com/lcs147/bmssp/blob/master/LICENSE
...
struct bmssp { // bmssp class
    ...
    BMSSPResult<wT> solve(int s) {
        ...
        int original_s = s;  // Save the original source node
        s = toAnyCustomNode(s);
        d[s] = 0;
        path_sz[s] = 0;
        ...
        // Convert back to real node space
        vector<int> predecessor_real(n);
        vector<wT> distance_real(n);
        
        for(int i = 0; i < n; i++) {
            distance_real[i] = d[toAnyCustomNode(i)];
            predecessor_real[i] = customToReal(pred[toAnyCustomNode(i)]);
        }
        
        return BMSSPResult<wT>{original_s, distance_real, predecessor_real};
    }
